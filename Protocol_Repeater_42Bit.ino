...
/* --------------------------------------------------------------------------------------
    Flags:
        ...
        |__________________________________ F_STATE         Sync detected, after sync data "0" or "1" lengths relevant
...
void PinChangeISR0()
{
    ...
    {
        //--- falling edge of sync impulse
        if (Time > (rise_Time + glitch_Length))
        {
            //--- no glitch, subtract last falling edge to get pulse time.
            ...
            {
                //--- looking for sync-impulse (640uS)
                if ((Time > sync_MIN) && (Time < sync_MAX))
                {
                    //--- sync length okay
                    build_Buffer[0] = 0;
                    ...
                    bit_Count = 0;
                    bitSet(isrFlags, F_STATE);              //--- Set data mode
                    digitalWrite(LED_EXTERN, LOW);          //--- debugging
                }
            }
